...
<Callout type="info">
  The default [`CloudStorage`](/docs/api/classes/CloudStorage) and [`CloudKVStorage`](/docs/api/classes/CloudKVStorage) APIs share provider options. Set the access token through either API and both defaults will use it. The `drive.appdata` scope is sufficient for key-value storage.
</Callout>
